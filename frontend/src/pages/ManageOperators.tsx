...
export default function ManageOperators() {
  ...
  const setPasswordMutation = useMutation({
    mutationFn: ({ id, password }: { id: number; password: string }) =>
      operatorService.setPassword(id, password),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["operators"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => operatorService.delete(id),
    ...
    return (
      <div className="space-y-6 animate-page-enter">
        <h1 className="text-2xl lg:text-3xl font-black text-foreground tracking-tight uppercase italic">
          Manage <span className="text-primary">Operators</span>
        </h1>
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
          <p className="text-red-500 font-bold">Failed to load operators. Check your connection.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1 font-medium">
            Add operators and set their passwords. Only admin and owner can manage operators.
          </p>
        </div>
        <button
          type="button"
              ...
              <tbody>
                {operators.map((op: Operator) => (
                  <tr key={op.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                    <td className="px-6 py-4 font-medium text-foreground">{op.full_name || "—"}</td>
                    <td className="px-6 py-4 text-muted-foreground">{op.phone || "—"}</td>
                    <td className="px-6 py-4 text-muted-foreground">{op.email}</td>
                    <td className="px-6 py-4 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          setPasswordForId(op.id);
                          setNewPassword("");
                        }}
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (window.confirm(`Remove operator "${op.full_name || op.email}"? They will no longer be able to sign in.`)) {
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black text-foreground uppercase tracking-tight">Add operator</h3>
              <button
                type="button"
                onClick={() => setAddOpen(false)}
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {createMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Add operator"}
                </button>
              </div>
            </form>
            {createMutation.isError && (
              <p className="text-sm text-red-500 font-medium">
                {(createMutation.error as { response?: { data?: { detail?: string } } })?.response?.data?.detail || "Failed to add operator."}
              </p>
            )}
          </div>
        </div>
