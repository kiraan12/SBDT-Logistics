...
export type PdfCopy = "all" | "booking" | "pod" | "office";

export default function PrintShipment() {
  const { id } = useParams<{ id: string }>();
  const { pathname } = useLocation();
  const queryClient = useQueryClient();
  const shipmentsPath = pathname.startsWith("/owner")
    ? "/owner/shipments"
  ...
  useEffect(() => {
    ...
    const fetchPdf = async () => {
      try {
        ...
        const baseName = filename?.trim() ? filename.trim() : `shipment_${id}_${pdfCopy}`;
  ...
  if (!hasValidId) {
    return (
      <div className="max-w-5xl mx-auto p-12 text-center space-y-6 animate-page-enter">
        ...
        <p className="text-muted-foreground max-w-sm mx-auto">The shipment ID is missing or invalid. Go back to the list and open a shipment to print.</p>
        <Link to={shipmentsPath} className="btn-premium px-10 py-4 rounded-2xl text-xs font-black uppercase tracking-widest inline-block">Back to shipments</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-20 animate-page-enter">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <Link
          to={`${shipmentsPath}/${shipment.id}/edit`}
          className="px-8 py-4 bg-muted border border-border rounded-2xl text-muted-foreground font-black uppercase tracking-widest text-[10px] hover:bg-muted/80 hover:text-foreground transition-all flex items-center gap-3 self-start md:self-auto"
        >
